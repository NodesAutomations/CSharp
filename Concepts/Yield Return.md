...
- Implementing state machines that produce a sequence of output values based on a sequence of input values.
    ...
    n C#, `yield return` is a language feature that allows a method to return a sequence of values one at a time, rather than returning all the values at once. The method that uses `yield return` is called an iterator method, and it's defined using the `yield` keyword along with the `return` keyword.
